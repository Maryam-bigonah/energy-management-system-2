...
class DeviceState:
    ...
    def turn_on(self, duration_minutes: float):
...
class LoadProfileGenerator:
    ...
    def __init__(
        ...
        # New: Track activations for Task Export
    ...
    def generate(self) -> (pd.DataFrame, List[Dict]):
        ...
        for step in tqdm(range(min_steps), desc="Simulating"):
            self._simulate_timestep(current_time, time_offset_minutes)
    ...
    def _simulate_timestep(self, current_time: datetime, time_offset_minutes: float):
        ...
        for device_inst in self.device_instances:
            ...
            # Handle regular devices (probabilistic with partial-interval)
            else:
                ...
                # Check if device should turn on
                if not device_inst.is_on:
                    should_turn_on = should_device_turn_on(
                        device_config,
                        current_time,
                        occupancy_prob,
                        multiplier,
                    ...
                    if should_turn_on:
                        ...
                        device_inst.turn_on(duration)
                        is_first_interval = True
                        
                        # LOG ACTIVATION for Shiftable Tasks (or all non-cycling/non-continuous)
                        if getattr(device_config, 'shiftable', False):
                            # Approximate energy for this activation (Power * Duration)
                            # Note: This is an estimated projection; actual energy varies with noise/peaks.
                            avg_power_kw = device_config.active_power_w / 1000.0
                            energy_kwh = avg_power_kw * (duration / 60.0)
                            
                            self.activations.append({
                                'task_id': f"{device_id}_{len(self.activations)}",
                                'device_id': device_id,
